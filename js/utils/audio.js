...
const Audio = (() => {
  ...
  function tone(freq, type, duration, vol = 0.4, startTime = 0) {
  ...
  const sfx = {
    ...
    wave()     { init();
                 [0,0.1,0.2].forEach(d => tone(600 + d*200, 'square', 0.12, 0.25, d));
               },
    levelUp()  { init();
                 [0,0.1,0.2,0.3].forEach((d,i) => tone(400 + i*150, 'sine', 0.15, 0.3, d));
               },
    gameOver() { init();
                 [0,0.15,0.3,0.5].forEach((d,i) => tone(300 - i*50, 'sawtooth', 0.2, 0.4, d));
               },
